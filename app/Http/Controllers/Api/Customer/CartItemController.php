...
class CartItemController extends Controller
{
    ...
    public function calculation(Request $request)
    {
        ...
        if (!$cart->isEmpty()) {
           ...
           //membership discount
            ...
            $ord_total = Order::where('customer_id', $this->customer->id)->sum('total');
            $payment_done = Payment::where('customer_id', $this->customer->id)->sum('received_amount');
            $wallet_rcv = Wallet::where('customer_id', $this->customer->id)->sum('receive_amount');
            $wallet_dedct = Wallet::where('customer_id', $this->customer->id)->sum('deducted_amount');
    ...
    public function insert(Request $request)
    {
        // brand dependency
        // if ($request->brand_id) {
        //     $cart = CartItem::where(['customer_id' => $this->customer->id, 'service_id' => $request->garment_id, 'service_type_id' => $request->service_type_id, 'brand_id' => $request->brand_id, 'color' => $request->color])
        //         ->whereIn('addon_id', [array($request->addon)])->first();
        // } else {
            if($request->addon){
                $cart = CartItem::where(['customer_id' => $this->customer->id, 'service_id' => $request->garment_id, 'service_type_id' => $request->service_type_id,])
                    ->whereIn('addon_id', [array($request->addon)])->first();
            }else{
                $cart = CartItem::where(['customer_id' => $this->customer->id, 'service_id' => $request->garment_id, 'service_type_id' => $request->service_type_id,])->first();
            }
        // }
        $rules = [
            'quantity' => 'required',
    ...
    {
        $cart = CartItem::where(['customer_id' => $this->customer->id, 'service_id' => $service_id])->first();
        return response()->json([
            'status' => 1,
            'data' => $cart,
        ]);
