...
class AddOrders extends Component
{
    ...
    public $payment_notes, $payment_mode,$cgst, $sgst, $cgst_percentage, $sgst_percentage, $orderbalance,$note,  $service_types, $contrycode, $service, $inputi, $prices = [],$quantity = [], $selected_type, $addons, $brands, $addonKey, $selected_addons = [], $colors = [], $assignaddon, $assignBrand, $brandKey, $selected_brands = [], $otherbrand = [];

    public $customer_name, $phone, $service_addon, $email, $tax_no, $gst, $salutation, $dob,$address, $company_name, $company_address, $locality, $pin, $custdiscount, $rating, $selected_customer, $customers,$custname, $custid, $addressnew, $flat, $area, $landmark, $latitude, $longitude, $addtype, $other, $custpincode, $voucher, $voucherid, $voucherdiscount, $vouamnt, $vouchercode, $voucer_id, $voucher_code, $vouchr_amount, $getvoucher, $customer_query,$is_active = 1;

    public $total,$sub_total, $discnt, $brand_name, $other_brands, $cashback, $addon_total, $tax_percent, $tax,$balance, $flag = 0, $lang, $categories, $service_category_id, $pickupoption, $deliveryoption, $getdiscount, $getcashback, $salutations, $membershipimg;
    ...
    public function mount()
    {
        ...
        $this->brands = Brand::where('is_active',1)->orderBy("brand_name", "asc")->get();
    ...
    public function updated($name,$value)
    {
        ...
        if($name == 'customer_query' && $value != ''){
            ...
            })->where('is_active', 1)->latest()->limit(5)->get();
    ...
    public function selectService($id)
    {
        ...
        if($this->service)
        {
            ...
            {
                $servicetype = ServiceType::where('id',$row->service_type_id)->where('is_active',1)->first();
                if($servicetype) {
    ...
    {
        $master_setting = MasterSettings::where('master_title', 'order_prefix')->where('is_active', 1)->first();
        $default_number = $master_setting->master_value .'-';

        $code_prefix =$default_number;
        ...
        if($ordernumber && $ordernumber->order_number !=""){
            ...
            $code=explode($default_number, $ordernumber->order_number);
            $new_code = $code[1] + 1;
    ...
    public function calculateTotal()
    {
        ...
        // $this->tax = $afterdis * $this->tax_percent/100;
    ...
    {
        $receive_amount = \App\Models\Wallet::where('customer_id',$this->custid)->sum('receive_amount');
        $deduct_amount = \App\Models\Wallet::where('customer_id',$this->custid)->sum('deducted_amount');
        $wallet = $receive_amount - $deduct_amount;
        $redeem_amount = MasterSettings::where('master_title', 'reedem_amount')->where('is_active', 1)->first();
        $reedmamnt = $redeem_amount->master_value;
        $amount = 0;
        $this->calculateTotal();
        $this->validate([
            'outlet' => 'required',
        ...
        {
            $this->addError('pickupoption','Select a pickup option');
            return 0;
        }

        if($this->deliveryoption == null)
        {
            $this->addError('deliveryoption','Select a delivery option');
            return 0;
        }

        if($this->delivery_outlet == null)
        {
            $this->addError('delivery_outlet','Select a delivery option');
            return 0;
        }

        /* if wallet balance is less than reedem amount */
        if($wallet < $reedmamnt && $this->payment_type ==6)
        {
            $this->addError('error','Min use amount for Lk credit is.'.$reedmamnt);
            return 0;
        }
        ...
        $orderdate = Carbon::parse($this->date)->toDateTimeString();
        $today = Carbon::today()->toDateString();
        $deliverydate = Carbon::parse($this->delivery_date)->toDateTimeString();

        if($orderdate < $today)
        {
            $this->dispatchBrowserEvent('alert', ['type' => 'error',  'message' => 'Can not create orders for old date.']);
            return 0;
        }

        if($deliverydate < $orderdate)
        {
            $this->dispatchBrowserEvent('alert', ['type' => 'error',  'message' => 'Delivery date can not be before the order date.']);
            return 0;
        }
        ...
        if(Auth::user()->user_type == 2 && $this->pickupoption == 1){
            $status = 2;
            $pickup_date = Carbon::parse($this->date)->toDateTimeString();
        }else{
            $status = 0;
        }
        ...
        if($this->flag == 0)
        {
            $order = Order::create([
                ...
                'pickup_option' => (int)$this->pickupoption,
                'delivery_option' => (int)$this->deliveryoption,
                ...
                'delivery_address_id' => (int)$this->deliveryaddress,
                ...
                'discount' => $this->discnt??0,
                'cashback_amount' => $this->getcashback ?? 0.00,
                'tax_percentage' => $this->tax_percent ?? 0,
                'tax_amount' => $this->tax ?? 0,

                'cgst_percentage' => $this->cgst_percentage ?? 0,
                'sgst_percentage' => $this->sgst_percentage ?? 0,
                'cgst_amount' => $this->cgst ?? 0,
                'sgst_amount' => $this->sgst ?? 0,
            ...
            foreach($this->selservices as $key => $value)
            {
                if($this->selected_brands !=[]){
                    if($this->selected_brands[$key] == "Others"){
                        $selectedbrand = $this->otherbrand[$key];
                    }else{
                    ...
                    if($selectedbrand){
                        ...
                        if($brand == null){
                            $brand_ids = Brand::create([
                                'brand_name' => $this->otherbrand[$key],
                                'is_active' => 1,
                            ])->id;
                        $select_brand = $brand_ids;    
                        }else{
                            $select_brand = $selectedbrand;
                        }
                    }
                }
            ...
            if($this->paid_amount && ($this->payment_type !=6||$wallet > $this->paid_amount))
            {
            ...
            if($this->paid_amount && $this->payment_type ==6)
            {
                if($wallet < $this->paid_amount){
                    \App\Models\Payment::create([
                        'payment_date' => $this->date,
                        'customer_id' => $this->selected_customer->id ?? null,
                        'customer_name' => $this->selected_customer->name ?? null,
            ...
            if($this->payment_type==6 && $this->paid_amount)
            {
                if($wallet > $reedmamnt){
                    if($wallet > $this->paid_amount){
                        \App\Models\Wallet::create([
                            'deducted_amount' => $this->paid_amount,
                            'order_id' => $order->id,
                            'customer_id' => $this->selected_customer->id ?? null,
                            'remarks' => 'Lk Credit transaction order placed',
                        ]);
                    }
                    if($wallet < $this->paid_amount){
                        \App\Models\Wallet::create([
            ...
            $this->dispatchBrowserEvent('alert', ['type' => 'success',  'message' => $order->order_number.' Was Successfully Created!']);
