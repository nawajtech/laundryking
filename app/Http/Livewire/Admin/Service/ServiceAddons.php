...
class ServiceAddons extends Component
{
    ...
    public function delete($id)
    {
        if (\App\Models\OrderAddonDetail::where('addon_id', $id)->doesntExist()) {
            ...
            $this->dispatchBrowserEvent('alert', ['type' => 'success',  'message' => 'AddOn deleted Successfully!']);
        } else {
            /* if addon has no children */
            $this->dispatchBrowserEvent('alert', ['type' => 'error',  'message' => 'Addon deletion restricted!']);
        }
        $this->addons = Addon::latest()->get();
    }
    //Reset Fields
