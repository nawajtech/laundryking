<div>
    ...
    <head>
        ...
        <title>{{$lang->data['settlement_report'] ?? 'Outstanding Report'}}</title>
        <style>
            #main {
    ...
    <body onload="">
        @php
            ...
            $orders = \App\Models\Order::addSelect(['received_amount_sum' => \App\Models\Payment::selectRaw('sum(received_amount) as total')->whereColumn('order_id', 'orders.id')->groupBy('order_id')])
        ...
        @endphp
        <h3 class="fw-500 text-dark">{{$lang->data['settlement_report'] ?? 'Outstanding Report'}}</h3>
            ...
            <tbody>
                @foreach($orders as $row)
                    <tr>
                        ...
                        <td style="width: 15%" >
                            <p class="text-xs px-3 font-weight-bold mb-0"> {{$row->total ?? 0}}</p>
                        </td>
                        @php
                            $rcv_amount=App\Models\Payment::where('order_id',$row->id)->sum('received_amount');
                        @endphp
                        <td style="width: 15%" >
                            <p class="text-xs px-3 font-weight-bold mb-0"> {{$rcv_amount ?? 0}}</p>
                        </td>
                        <td style="width: 15%" >
        ...
        <div class="col">
            ...
            <span class="text-sm text-dark ms-2 fw-600 mb-0">{{getCurrency()}}{{number_format($orders->sum('received_amount_sum'),2)}}</span>
        </div>
        </div>
        </table>
    </body>
