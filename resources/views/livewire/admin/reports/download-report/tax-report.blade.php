<div>
    ...
    <body onload="">
        @php
            /* sales */
            if ($category == 1 && $outlet) {
            $reports = \App\Models\Order::whereDate('order_date', '>=', $from_date)->whereDate('order_date', '<=', $to_date)->where('status', 3)->where('outlet_id',$outlet)->latest()->get();
        }elseif($category == 1){
            $reports = \App\Models\Order::whereDate('order_date', '>=', $from_date)->whereDate('order_date', '<=', $to_date)->where('status', 3)->latest()->get();
        }
        /* expense */
        if ($category == 2) {
            $reports = \App\Models\Expense::whereDate('expense_date', '>=', $from_date)->whereDate('expense_date', '<=', $to_date)->latest()->get();
        }
            $lang = null;
