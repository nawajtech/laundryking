<div>
    ...
    <body onload="">
        @php
            if ($outlet != 0 && $status != -1) {
                $orders = \App\Models\Order::whereDate('order_date', '>=', $from_date)->whereDate('order_date', '<=', $to_date)->where('outlet_id', $outlet)->where('status', $status)->latest()->get();
            } elseif($outlet != 0) {
                $orders = \App\Models\Order::whereDate('order_date', '>=', $from_date)->whereDate('order_date', '<=', $to_date)->where('outlet_id', $outlet)->latest()->get();
            } elseif($status != -1) {
                $orders = \App\Models\Order::whereDate('order_date', '>=', $from_date)->whereDate('order_date', '<=', $to_date)->where('status', $status)->latest()->get();
            } else{
        ...
        <table id="main" width="100%" cellpadding="0" cellspacing="0">
            <thead class="table-dark">
                <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xs  opacity-7">{{$lang->data['outlet'] ?? 'Outlet Code'}}</th>
            ...
            <tbody>
                @foreach ($orders as $row)
                    <tr>
                        ...
                        <td>
                            <p class="text-xs px-3 font-weight-bold mb-0">
                                {{ $row->outlet->outlet_code ?? 'Walk In Customer' }}</p>
                        </td>
