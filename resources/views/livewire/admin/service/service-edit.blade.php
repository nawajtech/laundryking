<div>
    <div class="row align-items-center justify-content-between mb-4">
        <div class="col">
            <h5 class="fw-500 text-white">{{ $lang->data['edit_garment'] ?? 'Edit Garment' }}</h5>
        </div>
        <div class="col-auto">
                ...
                <form>
                    <div class="card-body p-3 mb-1 mt-2">
                        <div class="row g-3 align-items-center">
                            <div class="col-md-3">
                                <label class="form-label">{{ $lang->data['expense_category'] ?? 'Category' }}<span class="text-danger">*</span></label>
                                @php
                                    $inline_categories = App\Models\ServiceCategory::latest()->get();
                                @endphp
                                <select class="form-control" wire:model="service_category_id">
                                    <option value="">{{ $lang->data['choose_service_category'] ?? 'Choose Category' }}</option>
                                    @foreach ($inline_categories as $row)
                                        <option value={{ $row->id }}>{{ $row->service_category_name }}</option>
                                    @endforeach
                                </select>
                                @error('service_category_id')
                                <span class="error text-danger">{{ 'Category Required' }}</span>
                                @enderror
                            </div>


                            <div class="col-md-3">
                                <label class="form-label">{{ $lang->data['service_name'] ?? 'Garment Name' }}</label>
                                <input type="text" required class="form-control" placeholder="{{ $lang->data['enter_service_name'] ?? 'Enter Garment Name' }}" wire:model="service_name">
                                @error('service_name')
                            ...
                            <div class="col-md-2">
                                <label class="form-label">{{ $lang->data['garment_code'] ?? 'Garment Code' }}</label>
                            ...
                            <div class="col-md-2">
                                <label class="form-label">{{ $lang->data['service_name'] ?? 'How many Pieces' }}</label>
                            ...
                            <div class="col-md-6">
                                <label class="form-label">{{ $lang->data['information'] ?? 'Information' }}</label>
                        ...
                        <div class="table-responsive">
                            <table class="table align-items-center mb-0">
                                ...
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xs opacity-7 ps-3">{{ $lang->data['service_type'] ?? 'Service Type' }}</th>
                                    <th class="text-uppercase text-secondary text-xs opacity-7 ps-3">{{ $lang->data['service_price'] ?? 'Service Price' }}</th>
                                ...
                                @foreach ($inputs as $key => $value)
                                    <tr>
                                        ...
                                        <td>
                                            <select class="form-select" wire:model="servicetypes.{{ $value }}">
                                        ...
                                        <td class="align-middle text-center">
                                            <input type="number" class="form-control" min="0" oninput="validity.valid||(value='');" placeholder="Service Price" value="100" style="width: 150px;" wire:model="prices.{{ $value }}">
                                            @error('prices.' . $value)
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="align-middle">
                                            <a href="#" class="text-danger fw-600" wire:click.prevent="remove({{ $key }},{{ $value }})">
                            ...
                            <div class="footer-button px-5">
                                <a href="#" wire:click="add({{ $inputi }})" class="badge badge-xs badge-success fw-600 text-xs">
                                    {{ $lang->data['add_service_type'] ?? 'Add Service Type' }}
                                </a>
                            </div>
                        </div>
                        <div class="p-3 mb-1 mt-2">
                            <hr class="bg-secondary">
                            <p class="fw-500 text-dark">Addons</p>
                    ...
                    <div class="card-footer p-2 mx-2">
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="active" checked wire:model="is_active">
                                <label class="form-check-label" for="active">{{ $lang->data['is_active'] ?? 'Is Active' }} ?</label>
                            </div>
                            <div>
                                <a href="{{ route('admin.service_list') }}"> <button type="button" class="btn btn-secondary">{{ $lang->data['cancel'] ?? 'Cancel' }}</button></a>
                                <button type="submit" class="btn btn-primary ms-4" wire:click.prevent="save">{{ $lang->data['submit'] ?? 'Submit' }}</button>
                            </div>
                        </div>
                    </div>
                </form>
                    ...
                    <div class="row">
                        @foreach ($files as $key => $value)
                            <div class="col-1 m-2  customwidth customhover1" wire:click="selectIcon({{ $key }})">
                                <img src="{{ asset('assets/img/service-icons/' . $value['path']) }}" class="img-fluid">
                            </div>
                        @endforeach
                    </div>
                </div>
